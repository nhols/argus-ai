...
from vid_analyser.api.runtime import get_app_state, set_active_config_state
...
async def get_config(request: Request):
    ...
    if state.run_config is None or state.run_config_version_id is None:
        raise HTTPException(status_code=404, detail="Config not initialized")
    return {
        "id": state.run_config_version_id,
        "config": state.run_config.model_dump(mode="json"),
    }
...
async def update_config(payload: ConfigUpdateRequest, request: Request):
    ...
    try:
        ...
        record = await state.db.insert_config(config=config_document, source=payload.source)
