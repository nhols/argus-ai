...
| `infra/environments/example/variables.tf` | Defines environment-level inputs such as region, droplet size, image, SSH CIDR, app directory, and swap size. |
| `infra/environments/example/terraform.tfvars.example` | Example values for a concrete deployment. Copy this to `terraform.tfvars` and fill in real values, including optional swap sizing. |
